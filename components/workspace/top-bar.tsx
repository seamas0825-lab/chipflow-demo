...
import { BarChart3, KanbanSquare, MessageSquareText, Radar, Search, Table2, UploadCloud } from "lucide-react"
...
export function WorkspaceTopBar() {
  const { view, setView, agentFeedOpen, setAgentFeedOpen, copilotOpen, setCopilotOpen, setDropzoneOpen } =
    useWorkspace()
  ...
  return (
    <div className="flex h-14 shrink-0 items-center gap-3 border-b border-border bg-background/95 px-4 backdrop-blur-sm md:px-6">
      ...
      <div className="relative ml-2 hidden max-w-xs flex-1 md:block">
        ...
        <Input placeholder="Search MPN, ref des, manufacturer..." className="h-9 rounded-lg pl-8 text-sm" />
      </div>
      ...
      <div className="ml-auto flex items-center gap-2">
        <Button
          variant="outline"
          size="sm"
          className="hidden gap-2 rounded-lg bg-transparent sm:inline-flex"
        ...
        <Button
          ...
          size="sm"
          className={cn("gap-2 rounded-lg", !agentFeedOpen && "bg-transparent")}
